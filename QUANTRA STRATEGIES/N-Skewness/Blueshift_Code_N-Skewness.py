...
def rebalance(context, data):
    ...
    for ticker in context.stocks:
        ...
        if ORDER_PRESENT == False and buy_signal == True:
            order_target_percent(ticker, 1)
            buy_signal = False
            ORDER_PRESENT = True

        elif ORDER_PRESENT==False and sell_signal==True:
            order_target_percent(ticker, -1)
            sell_signal = False
            ORDER_PRESENT = True

        elif ORDER_PRESENT==True and sq_off_signal==True:
            sq_off_signal = False
